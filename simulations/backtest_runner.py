...
class BacktestRunner:
    ...
    async def run_backtest(
        ...
        for i in range(lookback, len(data) - 1):
            ...
            if position is not None:
                ...
                if exit_reason:
                    ...
                    equity *= (1.0 + ret)  # simple compounding on notional
            ...
            if (
                ...
                and hasattr(self.websocket_broadcaster, 'broadcast_backtest_update')
                and (i % int(self.config['update_every']) == 0)
            ):
                try:
                    await self.websocket_broadcaster.broadcast_backtest_update({
        ...
        if self.websocket_broadcaster and hasattr(self.websocket_broadcaster, 'broadcast_backtest_update'):
            try:
                await self.websocket_broadcaster.broadcast_backtest_update({
                    'session_id': session_id,
                    'symbol': symbol,
